...
----------
	...
	p��te bmp vai pcx ja lataa kuvan sen perusteella. Virhett� pukkaa, jos tiedosto ei ole
	kumpaakaan tyyppi�.
					 ...
					 int max_colors)
{
	...
	if (PD_unload) {
		...
		LPDIRECTDRAW temp = NULL;				 // v�liaikainen rajapinta jolla haetaan uusin versio
		int i;
...
int PisteDraw_Buffer_Uusi(int leveys, int korkeus, bool video_muisti, UCHAR color)
{
	...
	if ((unsigned long)(leveys*korkeus) > hal_caps.dwVidMemFree) {		//tarkastetaan riitt��k� videomuistia
		video_muisti = false;
							    ...
							    int vasen, int yla, int oikea, int ala)
{
	...
	if (vasen < PD_buffers[lahde_index].klipperi.left)	//Tarkistetaan ett� dataa ei oteta l�hdebufferin
	...
	if (oikea>vasen && ala>yla)
	{
		...
		if (FAILED(PD_buffers[kohde_index].lpdds->BltFast(x, y, PD_buffers[lahde_index].lpdds, &source_rect,
			...
			PisteLog_Kirjoita("[Error] Piste Draw: Unable to flip surface!\n");
			return PD_VIRHE; // blitataan kaikki paitsi l�pin�kyvyysv�ri
		}
	}
